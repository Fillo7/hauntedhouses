...
public class MonsterFacadeImpl implements MonsterFacade{
    ...
    public Long createMonster(MonsterCreateDTO monsterCreateDTO) {
        ...
        abilities.forEach(ability -> monsterCreate.addAbility(getAbility(ability.longValue())));
    ...
    private Ability getAbility(Long id){
        if(id == null) throw new IllegalArgumentException("id of ability was null");
        Ability ability = abilityService.findById(id);
        if(ability == null) throw new IllegalArgumentException("ability with id: "+id.intValue()+" doesnt exist");
